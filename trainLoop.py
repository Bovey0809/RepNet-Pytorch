...
def MAE(y, ypred):
    ...
    return ae / yarr.flatten().shape[0]
...
def getStart(periodLength):
    ...
    tmp2 = tmp * idx
    return torch.argmax(tmp2, 1, keepdim=True)
...
def training_loop(n_epochs,
    ...
    currEpoch = prevEpoch if validate and not train else prevEpoch + 1
    for epoch in tqdm(range(currEpoch, n_epochs + currEpoch)):
        ...
        if train:
            ...
            fscore = 0
            a=0
            for i, (X, y) in enumerate(pbar, start=2):
        ...
        if saveCkpt:
            ...
            torch.save(checkpoint, f'checkpoint/{ckpt_name}{str(epoch)}.pt')
...
def trainTestSplit(dataset, TTR):
    trainDataset = torch.utils.data.Subset(dataset, range(int(TTR * len(dataset))))
